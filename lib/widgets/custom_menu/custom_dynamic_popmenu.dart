...
class SongMenuBuilder {
  ...
  static List<PopupMenuEntry<String>> _buildMenuItems({
    ...
    items.addAll([
      ...
      if (sl<MozAudioHandler>().isInQueue(pid!))
        const PopupMenuItem(
          value: 'remove_queue',
          height: 40,
          child: _MenuRow(Icons.queue_music, 'Remove from Queue'),
        ),
      if (!sl<MozAudioHandler>().isInQueue(pid))
        const PopupMenuItem(
          value: 'add_queue',
  ...
  static Future<void> _handleMenuAction({
    ...
    switch (action) {
      ...
      case 'delete_download':
        final result = await DeleteAudioFile.deleteFile(song.data);
      ...
      case 'play_next':
        final _mediaItem = song.toMediaItem();
        sl<MozAudioHandler>().playNext(_mediaItem);
        AppSnackBar.success(context, "${_mediaItem.title} will play next");

        break;

      case "remove_queue":
        AppSnackBar.success(context, "Removed from current Queue ");
        final _mediaItem = song.toMediaItem();
        sl<MozAudioHandler>().removeQueueItem(_mediaItem);
        break;
      case 'add_queue':
        final _mediaItem = song.toMediaItem();
        sl<MozAudioHandler>().addQueueItem(_mediaItem);
        AppSnackBar.success(context, "Added to current Queue ");
        break;
      case 'artist':
      case 'album':
        _betaInfo(context);
  ...
  static void _showPlaylistDialog(BuildContext context, SongModel song) {
    ...
    showOnlinePlaylistDalogue(context, songModel: song);
  }
