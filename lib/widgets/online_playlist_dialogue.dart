...
void showOnlinePlaylistDalogue(
  ...
  showDialog(
    context: context,
    builder: (dContext) => _PlaylistSelectionDialog(songModel: songModel),
  );
}
...
class _PlaylistSelectionDialog extends StatefulWidget {
  ...
  const _PlaylistSelectionDialog({required this.songModel});
        ...
        onSave: (name) async {
          ...
          if (playlistId != null) {
            await context.read<OnlinePlaylistCubit>().addSongToPlaylist(
              ...
              songId: widget.songModel.getMap["pid"] ?? '',
              thumbnailUrl: widget.songModel.getMap["image"] ?? '',
            );
