...
class AudioDownloadService {
  ...
  Future<void> downloadSong({
    ...
    if (Platform.isAndroid) {
      ...
      musicDir = Directory('/storage/emulated/0/Music/MozMusic');
      appDir = externalDir;
    } else if (Platform.isIOS) {
      final docDir = await getApplicationDocumentsDirectory();
      musicDir = Directory('${docDir.path}/Music');
      appDir = docDir;
    } else {
      final docDir = await getApplicationDocumentsDirectory();
      musicDir = Directory('${docDir.path}/Music');
    ...
    if (Platform.isAndroid) {
      ...
      songId = pathToId[filePath];

      if (songId == null) {
        log('Song not found in MediaStore, using file path hash as ID');
